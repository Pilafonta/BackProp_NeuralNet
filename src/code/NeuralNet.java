...
public class NeuralNet {
    ...
    public void train(double [][] inputvs, double [][] outputvs, double r) throws RuntimeException
    {
        ...
        for(int i = 0; i<inputvs.length; i++){
            ...
            //calculates the inputvs to get output
            double[] outPut = evaluate(inputvs[i]);

            
            List<Node> out = m_layers.get(m_layers.size()-1);
            for(int j = m_layers.size()-1; j>0; j--){
                //makes sure the first layer is the output layer
                if(j == m_layers.size()-1){
                    for(Node node: out){
                        //calculates the beta of the output layer
                        node.setBeta((outputvs[i][node.getPos()])-(outPut[node.getPos()]));
                    }
                }
                else{
                    //calculates the beta for the rest of the layers
                    List<Node>hidLayer = m_layers.get(j);

                    //List<Node> outPutLayer = m_layers.get(j+1);
                    for(Node hnode: hidLayer){
                        double sum = 0;
                        for(Connection con: hnode.getOutConnections()){
                            double outputTemp = con.getToNode().getOutput();
                            //calculates then sums up the betas for the rest of the layers
                            sum += (con.getWeight() * outputTemp)*(1 - outputTemp)* con.getToNode().getBeta();
                        }
                        hnode.setBeta(sum);
            ...
            //finds the difference between weights, then adds then to the current weight
